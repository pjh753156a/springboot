...
// JWT: 
// - Json Web Token, RFC 7519 표준에 정의된 JSON 형식의 문자열을 포함하고 있는 토근
// - 인증 및 인가에 사용
// - 암호화가 되어 있어 클라이언트와 서버 간의 안전한 데이터 교환을 할 수 있음
// - 헤더, 페이로드, 서명으로 구성됨
// - 헤더 : 토큰의 유형 (일반적으로 jwt)과 암호화 알고리즘이 포함되어 있음
// - 페이로드: 클라이언트 혹은 서버가 상대방에게 전달할 데이터가 포함되어 있음
// (작성자, 접근주체의 정보, 작성시간, 만료시간)
...
public class JwtProvider
{
    ...
    // 코드에 직접적으로 비밀키를 작성하는 것을 보안상 좋지 않음
    ...
    // 3. 외부 데이터 관리 도구를 사용
    // - 헤당 서버가 아닌 타 서버의 등록된 Vault 도구를 사용하여 비밀키를 관리
    ...
    public String create(String principle)
    {
        ...
        // 서명 (서명에 사용할 비밀키, 서명에 사용할 암호와 알고리즘)
